# AGENTS.md — load-bearing architecture notes

For future-Codex: things that look weird in the code but exist for a real reason. **Do not "simplify" these without testing on a real iPhone.** Every one of these notes corresponds to a multi-iteration debugging cycle that landed here for a specific reason.
...
## Common landmines for future-Codex
